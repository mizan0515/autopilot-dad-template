...
실세계 실패 패턴 (round-4): 릴레이가 이미 `governance_blocked` / `missing-evidence` 와 같은 신호를 generated dashboard 에 기록해 두었는데도 autopilot 측에서 그 신호를 **소비하지 않은** 채 정상 작업을 계속 진행하는 사례가 보고됐다 (15+ 시간 누락). 릴레이가 답을 알고 있어도 소비 루프가 끊기면 무용지물이다. F41 이 그 갭을 닫는다.
...
실세계 실패 패턴 (round-4): 다수의 운영적 실패 (runner retained-dirty 장시간 정체, 방치된 draft PR, 외부 런타임 브리지 미관측 등) 가 동시에 진행 중인데도 `FAILURES.jsonl` 이 **비어 있는** 사례가 있었다. 실패가 없었던 게 아니라 구조화된 행을 적지 않았기 때문에 다운스트림 도구가 트리아지할 게 없었다. F40 가 그 갭을 닫는다.
...
실세계 실패 패턴 (round-4 의 원래 동기): UX 가시 작업으로 라벨된 9 개 이상의 PR 이 어떤 런타임 캡처도 없이 머지됐고, STATE/HISTORY 는 "외부 브리지가 없어서 증거 없음" 같은 자기-면죄성 메모를 반복했다. 진짜 원인은 외부 브리지 부재가 아니라 **증거를 요구하는 게이트 자체가 없었던 것**. 외부 브리지는 Unity MCP / Claude Preview / Selenium / Playwright / 디바이스 시뮬레이터 / DB 인스턴스 등 프로젝트 형태마다 다르다. 이 섹션이 그 게이트의 에이전트-측 계약이고, `tools/Validate-RuntimeEvidence.ps1` 가 강제하는 검사다.
