...
iter 시작 직후:
- `run_id` 확보: 환경변수 `AUTOPILOT_RUN_ID` 가 있으면 그대로 사용, 없으면 UUID 생성 후 `$env:AUTOPILOT_RUN_ID` 에 export. (러너가 F37 규약대로 미리 세팅한다.)
- `.autopilot/LOCK` 에 `{pid, started_at, host, run_id, prompt: "lite"}` 기록.

종료 직전:
1. `METRICS.jsonl` 에 `{iter, ts, run_id, duration_s, outcome: "idle-upkeep|doc-only|escalated", prompt: "lite"}` 한 줄 append. **`run_id` 누락은 F38 ledger-consistency / F40 failures-logged 규약 위반.**
2. outcome 이 `escalated` 인 경우 같은 `run_id` 로 `FAILURES.jsonl` 에 한 줄 append:
   `{ts, run_id, event: "lite-escalation", result: "prompt-escalation-required", detail: "<reason>"}` (F40 규약: non-clean 결과는 구조화된 FAILURES 행 필요. `idle-upkeep` / `doc-only` 는 clean 이라 FAILURES 행 불필요.)
3. `NEXT_DELAY` 에 다음 대기 초 (60–3600) 기록.
4. `.autopilot/LOCK` 제거.
5. `.autopilot/LAST_RESCHEDULE` touch.

이 lite 계약은 full PROMPT.md 의 round-4/5 게이트 (F37 run_id, F38 ledger, F40 failures-logged) 의 **부분집합 (subset)** 이다 — lite mode 라고 게이트가 약화되지 않는다. F39 runtime-evidence / F41 DAD report consumption / F45 token-economy 는 lite 의 허용 작업 (idle-upkeep / doc-only) 본질상 트리거되지 않으며 검증기가 자동으로 skip 한다.
