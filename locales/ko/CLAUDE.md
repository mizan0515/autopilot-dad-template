...
- 프로젝트 고유의 배선을 주시한다 — 의존성 그래프, 설정 참조, 입력/이벤트 바인딩, 상태 소유권 경계, 런타임 부트스트랩 갭. (형태별 예: Python — DI 컨테이너 + 엔트리포인트; Node — 미들웨어 순서 + 설정 출처; Go — 패키지 init 순서 + 플래그 wiring; Unity — 씬/프리팹 참조 + Inspector wiring; Unreal — 블루프린트 참조; 웹 — 번들러 엔트리 + CSS 모듈 import.)
